...
void GrammarPool::buildLR1ParseTable()
{
    ...
    // // output First set
    // #ifdef _MSC_VER
    //     ofstream fres(".\\First.txt");
    // #else
    //     ofstream fres("./First.txt");
    // #endif
    // fres << "First sets:"<<endl<<endl;
    // for (int i = 0; i < this->ProdPool->NSymbolsFirsts.size(); i++)
    // {
    //     fres << this->NPool->getString(i) << ":" << endl << "\t";
    //     for (auto index : this->ProdPool->NSymbolsFirsts[i])
    //     {
    //         fres << this->TPool->getString(index) << " ";
    //     }
    //     if (this->ProdPool->hasEpsilonFirsts[i])
    //     {
    //         fres << "epsilon";
    //     }
    //     fres<<endl;
    ...
    Iterms Its0 = Iterms(sprod, this->ProdPool, -1);
    Its0.closure();
    this->ItsPool = new ItermsPool(0, Its0);
