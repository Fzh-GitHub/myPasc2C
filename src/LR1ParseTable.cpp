...
void LR1ParseTable::insertRow(const ItermsNode &node)
{
    for(auto iterm:node.iterms.Its)
    {
        if(iterm.pos==iterm.right.size())
        {
            if(iterm.forward!=-1)
            {
                if(ActionTable[node.index][iterm.forward].value==0)
                {
                    ActionTable[node.index][iterm.forward]=Action(Action::Reduce,iterm.prodIndex);
                }
            }
            else if(iterm.left!=0)
            {
                if(ActionTable[node.index][this->acol-1].value==0)
                {
                    ActionTable[node.index][this->acol-1]=Action(Action::Reduce,iterm.prodIndex);
                }
            }
            else
